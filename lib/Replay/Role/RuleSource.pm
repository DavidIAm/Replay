package Replay::Role::RuleSource;
...
requires qw(next first by_idkey);
...
Replay::Role::RuleSource - Interface for a provider of a set of objects of type Replay::BusinesRule

=head1 SYNOPSIS

package MyRuleSource;

use Moose;
with 'Replay::Role::RuleSource';
...
my $source = MyRuleSource->new( eventSystem => $es, rules => [ $RuleInstance, $otherrule  ] );

=head1 DESCRIPTION

The purpose of this abstraction is to allow the dramatic scaling of these rules   Not all rules can (or should) be in memory - or the same process - at the same time.
